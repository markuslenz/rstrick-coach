...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <Head :title="t('ui.trainings.result')" />
    
        <div class="px-4 py-4">
            <Table>
                <TableHeader>
                    <TableRow class="bg-gray-200">
                        <TableHead class="font-extrabold">{{ t('ui.trainings.question') }}</TableHead>
                        <TableHead class="font-extrabold">{{ t('ui.trainings.answer') }}</TableHead>
                        <TableHead class="font-extrabold">{{ t('ui.trainings.trick') }}</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    <TableRow>
                        <TableCell>{{ t('ui.judges.title') }}</TableCell>
                        <TableCell>{{ props.result.judge.name }}</TableCell>
                        <TableCell>{{ props.trick.judge.name }}</TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell>{{ t('forms.level') }}</TableCell>
                        <TableCell>{{ props.result.level }}</TableCell>
                        <TableCell>{{ props.trick.level }}</TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell>{{ t('forms.trick_name') }}</TableCell>
            ...
            <div class="container py-2 px-2 mx-0 min-w-full flex flex-col items-center">
                <div class="flex bg-green-200 px-4 py-4 rounded-lg" v-if="props.result.is_correct == true">
                    ...
                    <div class="flex-1 text-3xl font-extrabold text-green-800">{{ t('ui.trainings.correct') }}</div>
                
                </div>
                <div class="flex bg-red-200 px-4 py-4 rounded-lg" v-else>
                    <div class="flex-1"><Annoyed class="h-20 w-20 text-red-700" /></div>
                    <div class="flex-1 text-3xl font-extrabold text-red-700">{{ t('ui.trainings.wrong') }}</div>
                </div>
            </div>
            <div class="container py-2 px-2 mx-0 min-w-full flex flex-col items-center">
                <Button class="inline-flex items-center justify-center w-8/12 font-bold" size="lg">
                    <Link v-if="judgeID" :href="show(judgeID).url">{{ t('ui.trainings.next_trick') }}</Link>
